    ...
    >
        ...
        <div class="py-1 role="none">
            @foreach ($items as $item)
                <a href="" onclick="event.preventDefault(); document.getElementById('{{ $name }}').value='{{ $item }}'; document.getElementById('filter').submit();" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" role="menuitem" tabindex="-1" id="menu-item-0">{{ $item }}</a>
            @endforeach
        </div>
    </div>
</div>
